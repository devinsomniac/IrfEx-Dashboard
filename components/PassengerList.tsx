        ...
        <div>
            <div className='grid md:grid-cols-4 gap-3'>
                {passengerInput.map((passenger, index) => (
                    <div key={index}>
                        <label className='font-bold text-gray-600'>Enter Passenger {index + 1} name</label>
                        <Input placeholder='Enter Passenger Name' className='w-[220px]' onChange={(e) => handleInput(index,e.target.value)}/>
                    </div>
                ))}

            </div>
            {passengerInput.length < 9 ? 
            <Button className='bg-yellow-500 rounded-full text-black font-bold my-2 text-2xl hover:bg-yellow-500 hover:shadow-xl' onClick={handleAddPassenger}>
                <IoMdAddCircleOutline className='text-white hover:text-black' />
            </Button>
            : <p>*An Itenary can have maximum of 9 passengers</p>
            }
            </div>
    )
}
